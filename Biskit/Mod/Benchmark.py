...
class Benchmark:
	...
	-> pdb_if:  model, fitted iteratively (n=10)
	-> pdb:      model, without iterative fitting
	"""
	...
	fitted_model_if - PDBModel
	fitted_model_wo_if - PDBModel
	reference - PDBModel
	-> aprofile : list [], rmsd c-alpha per residues  
	"""
	...
	rmsd_aa_out = open('%s'%output_file, 'w')
	...
	-> rmsd_res: list [], rmsd per residues
	"""	
	rmsd_res = []
