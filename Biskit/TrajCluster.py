...
class TrajCluster:
    ...
    def __raveled( self ):
        ...
        return oldN.array( map( oldN.ravel, t.frames ) )
    ...
    def cluster( self, n_clusters, weight=1.13, converged=1e-11,
        ...
        """
        if aMask == None:
            aMask = oldN.ones( self.traj.getRef().lenAtoms() )

        if self.fc == None or force or self.fcWeight != weight \
           or self.n_clusters != n_clusters or oldN.any( self.aMask != aMask) \
    ...
    def memberships( self ):
        ...
        @return: oldN.array( n_clusters x n_frames )
        @rtype: array
        """
    ...
    def centers( self ):
        ...
        @return: oldN.array( n_clusters x n_atoms_masked x 3 )
        @rtype: array
        """
        lenAtoms = oldN.shape( self.fcCenters )[1] / 3
        return oldN.reshape( self.fcCenters, ( self.n_clusters, lenAtoms, 3))
    ...
    def centerFrames( self ):
        ...
        @rtype: [int]
        """
        return oldN.argmax( self.memberships(), 1 )
    ...
    def memberFrames( self, threshold=0. ):
        ...
        msm = self.memberships()
        maxMemb = oldN.argmax( msm, 0 )

        r = [oldN.nonzero( oldN.equal(maxMemb, i) ) for i in range(0, self.n_clusters)]
        r = [ x.tolist() for x in r ]

        ## same thing but now taking all above threshold
        ## -> same frame can end up in several clusters
        if threshold > 0.:
            r2 = [ oldN.nonzero( oldN.greater( l, threshold) ) for l in msm ]
    ...
    def avgRmsd( self, cluster, aMask=None, threshold=0. ):
        ...
        if len(oldN.ravel(rms)) == 1:
            ## was: return oldN.average(rms)[0], 0.0
            return oldN.average(rms), 0.0
        if len(oldN.ravel(rms)) == 0:
            return 0.0, 0.0

        return oldN.average( rms ), SD( rms )
