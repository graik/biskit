...
def norm ( v ):
    """
    Calculates the norm of a N3 vector.
...
def vectorangle( v1, v2 ):
    """
    Calculates the angle between two N3 vectors.
...
def rotation (alpha=0.,beta=0.,gamma=0.):
    ...
    @param alpha: Angle around z (rad)
    @type alpha: float
    @param beta: Angle around x (rad)
    @type beta: float
    @param gamma: Angle around y (rad)
    @type gamma: float
...
def vectpermatrix(a,b):
    """
    Performs matrix multiplication of N3 vector a and N3x3 matrix b.
    
    @param a: Vector to multiply.
    @type a: 3 elements list
    @param b: Matrix to be multiplied.
    @type b: 3*3 list of lists
    
    return: Multiplication result (a vector)
    @rtype: 3 float tuple
    """
    res = [0.,0.,0.]
...
def sphericalAngles( coord= [1.,0.,0.]):
    """
    @param coord: Vector from which we want to get the spherical angles. 
    @type coord: 3 float elements iterable.
    
    return: aX,aZ, rotation around x and z axis to get coord (in radians)
    @rtype: float tuple
    """
