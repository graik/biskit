...
class Test(BT.BiskitTest):
    ...
    def test_Creation(self):
        """Instantiation test """
        d = FRETEntity('mCerulean',T.testRoot() + "/fret/single.db",chromo_autodef = False)
        a = FRETEntity('mCitrine',T.testRoot() + "/fret/single.db",chromo_autodef = False)

        try:
            p = FRETPair(d,a,database= T.testRoot() + "/fret/pair.db")
        except:
    ...
    def test_Loading(self):
        ...
        d = FRETEntity("LoadTest",T.testRoot() + "/fret/single.db",chromo_autodef = False)
        a = FRETEntity("LoadTest",T.testRoot() + "/fret/single.db",chromo_autodef = False)
        d2 = FRETEntity("mCitrine",T.testRoot() + "/fret/single.db",chromo_autodef = False)


        # Not available db
        p = FRETPair(d,a, database= T.testRoot() + "/fret/lol.db")
        ...
        # No mandatory
        p = FRETPair(d2,a, database= T.testRoot() + "/fret/pair.db")
    ...
    def test_FretCalcs(self):
        ...
        d.chromophore = Chromophore(d.name,d.source,database = T.testRoot() + "/fret/chromo.db")
        a.chromophore = Chromophore(a.name,a.source,database = T.testRoot() + "/fret/chromo.db")
        ...
        self.assertAlmostEqual (p.getEnergyTransferRate(),0.295,2)
        self.assertAlmostEqual ( p.getEnergyTransferEfficiency(),0.5084,2)
        self.assertAlmostEqual ( p.getR0(),52.171,2)
