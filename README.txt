...
In this document:  * Release 2.0.0
		   * Installation
		   * License
		   * Open issues
...
$PYTHONPATH and depends on some other python libraries (Numeric,
Scientific, Biopython, biggles) which in turn depend on a few
additional libraries or programs (netcdf, egenix-mxtools, gnuplot).
...
packages can be found here:
   http://biskit.pasteur.fr/install/install/libraries/
...
Since we wanted to get this release out in time for the publication of
the application note, we didn't have time to fix every single
problem. The up-to-date list of open issues can be found at:
...
* (long) Docker.py test case fails: see bug

    Docker.py is used by the multidock.py script to parallelize hex
    docking runs.  Something is going wrong there. Expect problems
    until the bug is closed.  Classic "1 receptor agains 1 ligand"
    docking should still work.
...
* Amber* classes lack test cases
    ...
    We haven't yet written test cases for the AmberCrdParser,
    AmberCrdEntropist, AmberEntropist, AmberParmBuilder,
    AmberRstParser. These classes are used for the entropy
    calculations in the Gruenberg et. al (2006) Structure paper. They
    should work with Amber 8. I (Raik) plan to migrate them to Amber 9
    and write a proper entropy howto sometime in 2007.
...
* host management is a bit awkward
    ...
    memory hungry calculations. The resulting hosts.dat format is
    pretty awkward and not easy to maintain (or understand). There are
    three workarounds:
