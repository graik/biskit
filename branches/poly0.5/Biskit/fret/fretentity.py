...
"""
comments from Raik:
* tabs converted to space indent

* __init__: 'structure' is not documented
* it's good style to already define all fields of the instance in __init__ (with
  None or empty values) to show what one can expect

See Chromophore for some conceptual comment...
...
class FRETEntity:
	...
	Note that a FRET can be used for protein or non protein FRET entities definition.
	"""

    DEFAULT_DB = T.dataRoot()+'/fret/fret_prots_db/single_parameters.db' # default value for database file

    def __init__ (self,name="FRETEntity",database="", structure = None,chromo_autodef = False,verbose = True):
        """
        ...
        if self.name != "FRETEntity":
            ...
            if  self._loadFromDB(name,database) or not chromo_autodef :

                try:
                    self.chromophore = Chromophore(self.name,self.source,verbose = True)
                except:
...
class Test(BT.BiskitTest):
    ...
    def test_Loading(self):
        ...
        f = FRETEntity("LoadTest",T.testRoot() + "/fret/single.db",chromo_autodef = False,verbose=True)
        ...
        # Not available data
        f = FRETEntity("foooo",T.testRoot() + "/fret/single.db",chromo_autodef = False,verbose=False)
        self.assertEqual ( "name not found" in f.ehandler.lastWarning,True) 

        # Not available db
        f = FRETEntity("LoadTest",T.testRoot() + "/fret/lol.db",chromo_autodef = False,verbose=False)
        self.assertEqual ( "empty or badly formatted database" in f.ehandler.lastWarning,True) 

        #Mandatory params

        f = FRETEntity("Mandatory1",T.testRoot() + "/fret/single.db",chromo_autodef = False,verbose=False)
        self.assertEqual ( "mandatory parameters are not defined" in f.ehandler.lastError,True) 


        f = FRETEntity("Mandatory2",T.testRoot() + "/fret/single.db",chromo_autodef = False,verbose=False)
        self.assertEqual ( "mandatory parameters are not defined" in f.ehandler.lastError,True) 

    def test_TransitionMoment(self):
        """ Calculating transition dipole moment test"""
        try:
            f = FRETEntity(name ='mCerulean',chromo_autodef = False, database = T.testRoot() + "/fret/single.db",verbose = True)
            f.chromophore = Chromophore("mCerulean","2Q57",T.testRoot() + "/fret/chromo.db",verbose = True)
        except:
